...
public class UpdateUserTypeInteractor implements UpdateUserTypeInputBoundary {
    ...
    public void execute(UpdateUserTypeRequestModel request) {
        ...
        UserTypeDsResponseModel existingData = userTypeDsGateway.findById(request.id())
                .orElseThrow(() -> new UserTypeNotFoundException("Tipo de usuário não encontrado"));

        UserType userType = UserType.restore(existingData.id(), existingData.userType());
        ...
        outputBoundary.present(
                new UpdateUserTypeResponseModel(
                        update.id() ,
                        update.userType()
                )
        );
